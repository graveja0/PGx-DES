...
Get_ICER <- function(result) {
  ...
  result %>% full_join(out1,"strategy") %>% full_join(dominated.strong,"strategy") %>% full_join(dominated.extended,"strategy") %>% arrange(desc(dQALY)) %>% 
    mutate(dominated.extended = ifelse(is.na(dominated.extended),0,dominated.extended))
}
...
TornadoAll <-function(Strategies,Parms,Outcomes){
  ...
  strategyColors <- c("red","darkgreen","blue","black")
  
  #label.map <- cbind.data.frame(n=strategyNames,t=strategyColors)
  ...
  for (p in 1:nParams){
    ...
    for (x in seq(xStart,xEnd, by = xStep)){
      ...
      if (yOpt != yOptOld | x == xEnd){
        ...
        ylevel[nRect] <- p
        colRect[nRect] <- strategyNames[yOptOld]
        yOptOld <- yOpt
        y1 <- yOptOutcomes
      }
    }
...
TornadoAll2 <-function(Strategies,Parms,Outcomes){
  ...
  strategyNames<-Strategies
  strategyColors <- Strategies#c("red","darkgreen")
